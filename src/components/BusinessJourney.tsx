      ...
      <div className="flex flex-col items-center px-4">
        ...
        <div className="flex flex-col items-center gap-2 mb-10 text-center">
          <div className="flex flex-col md:flex-row items-center gap-4">
            <img src={best} alt="best-tally" className="h-20 drop-shadow-md" />
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800 tracking-wide">
  Unlock <span className="text-blue-700">Smarter Growth</span> for Your Business
</h2>
        ...
        <div className="w-full max-w-7xl overflow-x-auto">
          <div className="min-w-[900px] md:min-w-full border-[6px] border-black rounded-full py-6 px-4 flex md:flex-wrap justify-between items-center bg-white shadow-xl gap-y-6">
            {journeyData.map((item, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center text-center w-28 mx-2 group transition-transform duration-300 hover:scale-110"
              >
                <img
                  src={item.icon}
                  alt={item.alt}
                  className="h-20 w-20 mb-3 rounded-full object-cover shadow-lg"
                />
                <span
                  className={`w-3 h-3 rounded-full ${item.color} mb-1 shadow-md`}
                ></span>
                <p className="text-[11px] font-medium text-gray-700 group-hover:text-black leading-tight">
                  {item.label}
                </p>
              </div>
            ))}
